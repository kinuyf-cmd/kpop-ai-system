...
TEMPLATES = {
    ...
    # ⑧ 解説・分析（2026-04-16改修: {artist}+{number} 必須化・汎用句削除）
    "analysis": [
        "{artist}\n{number}冠の衝撃",
        "{artist}\n{number}ぶり記録",
        "{artist}\n戦略の全貌",
        ...
        "{artist}\n記録の裏側",
        "{artist}\n急騰の真相",
        # アーティスト名不在時の最低限テンプレ（旧「3市場の真相判明」等は削除）
        "結論が出た\n勝因はここ",
        "業界が驚いた\n急騰の真相",
        "なぜ勝てた\n全容がこれ",
    ],
...
GENRE_KEYWORDS = {
    "travel": ["旅行", "ソウル", "カフェ", "聖地巡礼", "観光", "グルメ", "ポップアップ",
               "聖地", "ホテル", "韓国旅行", "現地"],
    "fashion": ["ファッション", "着用", "コーデ", "ブランド", "即完売", "outfit",
                "コレクション", "私服", "スタイル"],
    ...
    "beginner": ["初心者", "入門", "5分で", "無料", "簡単", "ハウツー", "ガイド", "完全ガイド"],
...
def extract_number(title: str) -> str | None:
    # 年号（2020〜2035）を除外してから数字を抽出
    _no_year = re.sub(r'20[2-3]\d', '', title)
    m = re.search(r'(\d+(?:\.\d+)?(?:万|億|冠|ヶ月|か月|日|曲|公演|回|位|選|連|週|ぶり))', _no_year)
    if m:
        return m.group(1)
    m = re.search(r'(\d+)', _no_year)
    if m:
        n = int(m.group(1))
        if 1 <= n <= 999:  # 年号サイズを除外
            return m.group(1)
