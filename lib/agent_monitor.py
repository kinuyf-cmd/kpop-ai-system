...
AGENT_MASTER = {
    ...
    "mewtwo":               ("ミュウツー",      "CEO・戦略統合",             "CORE",         "breaking=step0 / strategy=step7"),
    "deoxys":               ("デオキシス",      "編集長（編集制作部長）",     "CORE",         "breaking=step1,2 / strategy=step8"),
    ...
    "arceus":               ("アルセウス",      "CQO（監査品質部長・総監督）","CORE",         "breaking=step4 / strategy=step14"),
    "butterfree":           ("バタフリー",      "トレンド情報収集",           "CORE",         "strategy=step1"),
    "lapras":               ("ラプラス",        "CMO（SEO分析部長）",         "CORE",         "strategy=step2"),
    ...
    "porygon_z":            ("ポリゴンZ",       "CTO（システム運用部長）",    "SUPPORT",      "ai_company"),
    "lugia":                ("ルギア",          "COO（経営企画部長）",        "SUPPORT",      "weekly_review=step2"),
    "meowth":               ("ニャース",        "CFO（財務管理部長）",        "SUPPORT",      "ai_company"),
    ...
    # illumise: UI/UX A/Bテスト担当（2026-04追加）
    "illumise":             ("イルミーゼ",      "UI/UX最適化（A/Bテスト）",   "SUPPORT",      "独立実行（cron週次）"),
    # raikou_thumb: サムネイル2行コピー専用（2026-04追加）
    "raikou_thumb":         ("ライコウ",        "サムネイル2行コピー生成",    "CORE",         "全パイプライン共通（サムネ生成直前）"),
    ...
    "deoxys_kpop":          ("デオキシス",      "編集長（編集制作部長）",     "CORE",         "breaking=step1,2 / strategy=step8"),
...
def _auto_discover_agents_from_files() -> None:
    """agents/*.md から AGENT_MASTER 未登録のエージェントを自動取り込み。

    YAML frontmatter の ROLE_CLASS / PIPELINE_POSITION / description と、
    見出し行 "# カタカナ名（...）" からメタデータを抽出し、AGENT_MASTER に追加する。
    既存エントリは上書きしない（手動キュレーションを優先）。
    これにより agents/ に新しい .md が追加されるだけで次回ダッシュボード更新時に反映される。
    """
    if not AGENTS_DIR.exists():
        return
    for path in sorted(AGENTS_DIR.glob("*.md")):
        # _kpop サフィックスは既存エイリアスで扱う。本体IDのみ発見対象
        base_id = path.stem
        canonical = KPOP_ALIAS_TO_CANONICAL.get(base_id, base_id)
        if canonical in AGENT_MASTER or base_id in AGENT_MASTER:
            continue
        try:
        ...
        role_class = "SUPPORT"
        pipeline_pos = "未接続（定義のみ）"
        description = ""
        for line in fm.splitlines():
            if line.startswith("ROLE_CLASS:"):
                role_class = line.split(":", 1)[1].strip() or "SUPPORT"
            elif line.startswith("PIPELINE_POSITION:"):
                pipeline_pos = line.split(":", 1)[1].strip() or pipeline_pos
            elif line.startswith("description:"):
                description = line.split(":", 1)[1].strip()
        # カタカナ名を見出しから抽出: "# カタカナ名（...）" or "# カタカナ名"
        ja = base_id
        h_match = re.search(r"^#\s*([^\n（(]+?)\s*[（(]", body, re.MULTILINE)
        if h_match:
            ja = h_match.group(1).strip()
        else:
            h_match2 = re.search(r"^#\s*(.+)$", body, re.MULTILINE)
            if h_match2:
                ja = h_match2.group(1).strip()
        # role: descriptionの先頭から短く切り出す（最大30文字）
        role = (description[:30] + "…") if len(description) > 30 else (description or "自動登録エージェント")
        AGENT_MASTER[base_id] = (ja, role, role_class, pipeline_pos)


# モジュール読み込み時に未登録エージェントを自動発見
_auto_discover_agents_from_files()
...
def build_org_map(metrics: dict) -> dict:
    ...
    return {
        ...
        "ceo": {
            ...
            "name_ja": "ミュウツー",
            "title": "CEO",
            "role": "戦略統合・経営意思決定（記事編集は編集長デオキシスに委任）",
        },
        "owner": {
            "name": "人間オーナー",
        ...
        "meeting_departments": _load_meeting_departments(agents_list),
    }


def _load_meeting_departments(agents_list: list[dict]) -> dict:
    """config/meeting_departments.json を読み込み、各部署のメンバー詳細を解決して返す。
    org_map.json の departments（役割クラス別）とは次元が異なる業務機能別部署。"""
    import json
    from pathlib import Path
    cfg_path = Path(__file__).resolve().parent.parent / "config" / "meeting_departments.json"
    ...
    try:
        ...
        return {}
    agent_idx = {a["id"]: a for a in agents_list}
    # suffix正規化（jirachi_kpop→jirachi等のエイリアスも拾う）
    for a in agents_list:
        base = a["id"].replace("_kpop", "")
        agent_idx.setdefault(base, a)

    result = {}
    for dept_id, dept in cfg.get("departments", {}).items():
        members_resolved = []
        for mid in dept.get("members", []):
            a = agent_idx.get(mid) or agent_idx.get(mid.replace("_kpop", ""))
            if a:
                members_resolved.append({
                    "id": a["id"], "name_ja": a["name_ja"],
                    "role": a["role"], "status": a.get("status", ""),
                })
            else:
                members_resolved.append({"id": mid, "name_ja": "?", "role": "(未登録)", "status": ""})
        result[dept_id] = {
            "label_ja": dept.get("label_ja", dept_id),
            "purpose": dept.get("purpose", ""),
            "cadence": dept.get("cadence", ""),
            "meeting_script": dept.get("meeting_script", ""),
            "discord_channel": dept.get("discord_channel", ""),
            "lead": dept.get("lead", ""),
            "lead_title": dept.get("lead_title", ""),
