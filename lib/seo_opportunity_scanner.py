...
def is_intent_mismatch(pos, ctr, imp):
    ...
    2026-09-10 追記(pos閾値 5.0 → 8.0):
      「相関図」クエリはゼロクリック検索(SERP画像パックで用が済む)。
      同一記事内で相関図クエリ pos2.7/CTR0.50% に対し、相関図以外は pos18.0 でも
      CTR6.53% と13倍。記事側要因ではなくクエリ側の性質。
      `恋は飴模様 相関図` が pos5.4/CTR0.51% で旧閾値をすり抜けていたため 8.0 へ。
      pos5-10帯の実測CTRベースラインは 2.68% あり、その 1/5 未満(<1%)は
      押し上げでは回収できない。
    """
