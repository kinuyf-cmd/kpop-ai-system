...
- **人名(K-POPアーティスト・俳優・スタッフを問わず)の英語/韓国語/日本語表記揺れ**。
  ...
  **英語ソース(HanCinema/Soompi等)のローマ字を根拠に「日本語表記が誤り」と指摘してはならない。**
  ...
  それらで確認できない場合は指摘しない
- **作品の英題・邦題・副題の「実在性が確認できない」という指摘**。
  検索で出てこないことは存在しないことの証明にならない。
  実例: 『Agent Kim Reactivated』を「英語副題の実在性がweb検索で確認できず」として
  critical にしたが、英語版Wikipedia/Netflix/Forbes に存在する正式タイトルだった。
  タイトルの表記を問題にするのは、**明確に別作品を指している場合のみ**。
