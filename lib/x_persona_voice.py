...
# 直近に選ばれた書き方が「?で終える」形だったか(_build_prompt が設定)。
_LAST_FORM_WANTS_QUESTION = False


def _has_question(text: str) -> bool:
    """疑問符(半角/全角)で読者に開かれているか。2026-08-20 エンゲージ率対策。"""
    return "?" in (text or "") or "？" in (text or "")


# 疑問形の文末表現。モデルは意味的には問うているのに「〜だろう。」「〜かな…」と
# 句点/三点リーダで閉じ、読者には問いとして届かない(実測: form固定でも追従2/6)。
# 生成をやり直すより、末尾の閉じ記号を「?」に替える方が確実で副作用も小さい。
_Q_TAIL = re.compile(
    r'(のかな|のかなぁ|かな|かなぁ|だろう|だろうか|んだろう|んだろ|でしょうか|'
    r'どうなんだ|どう思う|どうだろう|なんだろう|のだろうか|ますか|ですか)'
    r'\s*[。．\.…‥、]*\s*$'
)


def _force_question_mark(text: str) -> str:
    """疑問形で終わっているのに ? が無い文末を ? に直す。既に ? があれば何もしない。"""
    t = (text or "").rstrip()
    if not t or _has_question(t):
        return text
    m = _Q_TAIL.search(t)
    if not m:
        return text
...
_POST_FORMS = (
    ...
    "一つだけ素朴な疑問を書く。答えや推測は書かない。必ず『?』で終える。",
    # 2026-08-20: 実測でPhase2期間17投稿の疑問符0%・総いいね4/総リプ2(エンゲージ率0.7%)。
    #   設計思想は「コメント誘発が最重要」なのに、出力が全て独り言で閉じており
    #   読者に話しかけていなかった。engagement bait(「どう思う?」)は引き続き禁止のまま、
    #   「自分の体感を述べたうえで、他人の体感を尋ねる」形を追加して反応の余地を作る。
    "自分がどう感じたかを書いたあと、他の人はどうだったかを一言だけ尋ねる。『?』で終える。",
    "知らないことを正直に書いて、詳しい人に教わる姿勢で短く尋ねる。『?』で終える。",
    "二択で迷っている状態を書き、どちらだと思うかを尋ねる。『?』で終える。",
    ...
    # 2026-08-20 owner指定(型A): 本文=感想(1文)→起(+承転)、結はリプに残す。
    #   結まで本文に書くとリプ(URLのある側)を読む理由が消える。
    #   構成の指定はプロンプトだけでは守られないため、
    #   lib/x_traffic_picker.check_hook_structure_type_a() でも検査している。
    "まず自分の感想を1文だけ書き、次に何が起きたかを書く。"
    "理由や結論(だから/つまり/なぜなら)は書かず、そこで止める。",
    "感想を1文置いてから、状況を順に2文で書く。オチや答えは書かないまま終える。",
)
...
def _build_prompt(kind: str, context: dict, writer_key: str,
    ...
    # 2026-08-20: 「?で終える」形が選ばれたかをコード側で保持する。プロンプトだけでは
    #   上の【実測で多すぎた型】(問いかけ締めの禁止)と衝突して黙る側に倒れるため、
    #   採否は生成後に _needs_question で機械的に検査する。
    global _LAST_FORM_WANTS_QUESTION
    _LAST_FORM_WANTS_QUESTION = ("『?』で終える" in form)
    ...
    system = (
        ...
        "\n- engagement bait(記事と無関係に「どう思う?」「教えてください」とだけ付ける類)・"
        "煽り(「衝撃の」)・meta(「本記事では」)は禁止"
        # 2026-08-20: bait禁止を「疑問符禁止」と解釈した結果、17投稿すべてが独り言で閉じ
        #   エンゲージ率0.7%まで落ちた。禁止したいのは"中身のない問いかけ"であって、
        #   話題に即した自然な問いは推奨。ここを言い分けないとモデルは安全側に倒れて黙る。
        "\n- ただし『今回の書き方』が問いを求めている場合は、必ず『?』で終えること。"
        "話題に即した具体的な問いは bait ではない(禁止なのは中身のない定型の問いかけ)"
...
def generate_persona_post(context: dict, *, kind: str = "conversation",
    ...
    for attempt in range(3):
        ...
        #   解釈して全て独り言で閉じていたため、ここで機械的に弾いて再生成させる。
        if _LAST_FORM_WANTS_QUESTION:
            # 意味的に問うているのに閉じ記号で終わっている場合は ? に直してから判定。
            cand = _force_question_mark(cand)
            if attempt < 2 and not _has_question(cand):
                continue
        body = cand
        break
