...
def translate_ko_to_ja(text, context='K-POP entertainment news'):
    ...
    system = ("あなたはK-POP専門メディアの韓国語→日本語翻訳者です。以下のガイドラインで自然な日本語に翻訳してください。\n\n"
              ...
              "【Lv2追加ルール】\n"
              "- グループ名は日本で定着した英語表記で書く (IVE / BLACKPINK / SEVENTEEN)。ハングルを出力に含めない\n"
              "- 個人名(メンバー名・俳優名)はカタカナ表記にする。ローマ字表記は禁止\n"
              "  例: 장원영→チャン・ウォニョン (Jang Wonyoung は不可) / 정해인→チョン・ヘイン\n"
              "  ただし芸名が英字で定着している場合のみ英字可 (V / JIN / RM / Rain)\n"
              "- 地名・機関名・事務所名は日本語表記(漢字/カタカナ)にする。ハングルのまま残さない\n"
