"""
プロフィールガイド記事（カテゴリ112）再監査エージェント

機能:
1. profile_guide_review.jsonl から next_review_due が今日以前の記事を抽出
...
Usage:
  python3 lib/profile_guide_reviewer.py --dry-run
  python3 lib/profile_guide_reviewer.py --all          # 期限に関係なく全件チェック
  python3 lib/profile_guide_reviewer.py                # 期限到来分のみ
  python3 lib/profile_guide_reviewer.py --post-id 2475 # 単記事チェック
"""
...
LEDGER = BASE / "logs" / "profile_guide_review.jsonl"
LOG_PATH = BASE / "logs" / "profile_guide_review.log"
WP_API = "https://www.kpopjournal.tokyo/wp-json/wp/v2/posts"

HUB_POST_ID = 2485  # 112ハブ記事はinvertルール(欠如チェック)の対象外

# ── リスクに応じた次回監査間隔 ──────────────────────────────
REVIEW_INTERVALS = {
    "profile_guide":      {"high": 14, "medium": 30, "low": 60},
    "comprehensive_guide": {"high": 14, "medium": 30, "low": 60},
}
DEFAULT_INTERVAL = 30
...
CHECK_RULES = [
    ...
    {
        "id": "missing_hub_link",
        "pattern": r"kpop-hub-link|kpop-profile-hub-2026",
        "severity": "high",
        "message": "112ハブへの導線が見当たらない（kpop-hub-linkクラス不在）",
        "invert": True,
    },
    {
        "id": "missing_view_guide_link",
        "pattern": r"kpop-view-guide|kpop-streaming-guide-2026",
        "severity": "high",
    ...
    },
    {
        ...
        "pattern": r"ヘリン(?!.*ハリン)",
        "severity": "high",
        "message": "誤メンバー名「ヘリン」残存。正: ハリン(Haerin) ※NewJeans",
    },
    {
    ...
    {
        "id": "activity_overassert",
        "pattern": r"(現在活動中です|確実に活動|必ず出演|絶対に)(?!.*予定|.*とされています|.*と伝えられています)",
        "severity": "high",
        "message": "活動状況の断定表現。「〜とされています」「〜予定」等に緩和すること",
    },
    {
        "id": "newjeans_legal_overassert",
        "pattern": r"NewJeans.{0,100}?(完全復帰|全員活動|5人全員)",
        "severity": "critical",
        "message": "NewJeansの完全体活動を断言。2026年4月時点で未確定のため断言禁止",
    },
]
...
def review(dry_run=False, all_records=False, target_post_id=None):
    ...
    if not targets:
        _log(f"[profile_guide_reviewer] 期限到来記事なし (today={today})")
        return

    _log(f"[profile_guide_reviewer] 監査対象: {len(targets)}件")
    ...
    for rec in targets:
        ...
        if findings:
            ...
            if not dry_run:
                ...
                msg = (
                    f"{severity_icon} **プロフィールガイド記事 再監査アラート**\n"
                    f"[{pid}] {title}\n"
            ...
            _log(f"    ✅ 問題なし")
            if not dry_run:
                article_type = rec.get("article_type", "profile_guide")
                risk_level = rec.get("risk_level", "medium")
                interval = REVIEW_INTERVALS.get(article_type, {}).get(risk_level, DEFAULT_INTERVAL)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="プロフィールガイド記事 再監査スクリプト")
