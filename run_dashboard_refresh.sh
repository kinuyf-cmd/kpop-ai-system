...
# ─── フェーズ0: AI組織マップ再構築（agent_monitor.py） ───
# agents/*.md への追加／削除を org_map.json / agent_metrics.json に自動反映させる
log "フェーズ0: agent_monitor.py で組織マップを再生成..."
if python3 "${BASE_DIR}/lib/agent_monitor.py" >> "${LOG_FILE}" 2>&1; then
    log "フェーズ0: OK — org_map.json 再生成完了"
else
    log "WARNING フェーズ0: agent_monitor.py 失敗 (exit=$?) — 既存 org_map.json で続行"
fi
