...
function kpop_inject_internal_links( $content ) {
	...
	foreach ( $segments as $i => $seg ) {
		...
		foreach ( $cache as $title => $slug ) {
			...
			// キーが ASCII 英数字を含むときだけ ASCII 単語境界を使う。
			//
			// 日本語キーは以前「境界チェック無し(3文字以上+長い順優先で抑止)」だったが、
			// これでは部分一致を防げなかった。実害(2026-07-16 本番で確認):
			//   「イ・ソンミン」(俳優)の中の「ソンミ」を拾い、無関係な歌手 SUNMI へ誤リンク。
			// 3文字ルールは日本語では機能しない(カタカナ別名は3文字以上が普通)ため、
			// 前後がカタカナ・長音・漢字・半角カナなら「同じ語の続き」とみなしてマッチさせない。
			// ひらがな(助詞「が」「の」)・句読点・空白・行頭行末は語の切れ目として許可するので、
			// 「ソンミが新曲」「歌手のソンミ、」は従来どおりリンクされる。
			// これにより $kpop_ambiguous_names(ループ/カラ/ライズ 等)の誤マッチも構造的に解消する
			// (「無限ループする」「カラオケ」「サンライズ」はいずれも前後がカタカナ/漢字で弾かれる)。
			if ( preg_match( '/[A-Za-z0-9]/', $title ) ) {
				$pattern = '/(?<![A-Za-z0-9])(' . preg_quote( $title, '/' ) . ')(?![A-Za-z0-9])/u';
			} else {
				// カタカナ(全角/半角)・長音・漢字 = 語の内部とみなす文字クラス
				$jp_word = '\x{30A1}-\x{30FA}\x{30FC}\x{4E00}-\x{9FFF}\x{FF66}-\x{FF9D}';
				$pattern = '/(?<![' . $jp_word . '])(' . preg_quote( $title, '/' )
				           . ')(?![' . $jp_word . '])/u';
			}
