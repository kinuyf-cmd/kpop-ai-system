...
queue_decision() {
  # queue_decision SEVERITY CATEGORY DESCRIPTION RECOMMENDED RATIONALE OPTION_A OPTION_B
  local sev="$1" cat="$2" desc="$3" rec="$4" rat="$5" opt_a="$6" opt_b="$7"
...
# ─── 統計初期化 ─────────────────────
declare -i N_TOTAL=0 N_AUTO_REPAIRED=0 N_QUEUED=0 N_FAILED=0 N_SKIPPED=0
