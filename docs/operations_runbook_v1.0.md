...
| エージェント | 役割 | KPI |
|------------|------|-----|
| ミュウツー (mewtwo) | **CEO**・戦略統括・経営意思決定 | 日次方針の具体性、投資判断の適切さ |
| デオキシス (deoxys) | **編集長**（編集制作部長）・記事テーマ最終決定＋執筆 | 明日テーマの適切さ、記事品質 |
| アルセウス (arceus) | 監査品質部長・総監督・最終品質承認 | 投稿承認精度 > 95%、品質スコア80点以上の記事のみ公開 |
...
|---|-------------|-----------|----------------|--------|------------|----------------|
| 1 | mewtwo | CORE | breaking=step0 / strategy=step7 | 全レポート統合・テーマ戦略推奨（CEO）→編集長デオキシスへ申し送り | [breaking]なし→deoxys / [strategy]venusaur→deoxys | 不可（テーマ指定モードはskip可） |
...
| mewtwo | **不可**（テーマ指定モードはskip） | CEO戦略判断は代替不可 |
