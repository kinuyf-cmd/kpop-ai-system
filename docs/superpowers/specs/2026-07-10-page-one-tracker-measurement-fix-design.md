...
## 本書き込みの扱い（2026-07-10 の判断）

実装完了時点で、当日分の週次 cron（金 6:15）は既に旧スキーマ16行を書き終えていた。
ここへ新スキーマ17行を追記すると同一週に33行が並ぶ。旧行は `delta_basis` /
`clicks_abs` を持たないため rollback・前週比の計算には一切入らず無害だが、
feedback_loop の theme 別集計で当週だけ `unknown` が16件水増しされる。

計測の連続性を優先し、**当日の手動書き込みは行わず来週の cron に委ねた**。
progress は 122 行のまま。cron 3本は引数なしで呼ばれており、修正後のコードが
そのまま効く（tracker は本書き込み、rollback は dry-run 固定）。cron 変更は不要。

## 移行期の既知の挙動（3週間）

progress の過去122行は theme を持たないため、feedback_loop は直近4週の窓から
旧行が抜けるまで `theme=unknown n=77` を報告し、同じ「unknown は効果薄」提案を
毎週出す。これは計測バグの再発ではなく移行期の想定内。

`tools/reject_stale_proposals.py` を冪等化してあるので、毎週流して却下してよい
（`rejects_ts` で打ち消し済みの提案は再検出しない）。3週後に自然解消する。
